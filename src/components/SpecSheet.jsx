export default function SpecSheet({ title, meta, sheet, rows }) {
  return (
    <div className="blueprint border border-primary-200/70 text-primary-600">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-[1.4fr_1fr_0.6fr] border-b border-primary-200/70">
      ...
      {rows.map((row, i) => (
        <div
          key={i}
          className={`grid grid-cols-[auto_1fr] sm:grid-cols-[60px_260px_1fr] gap-4 sm:gap-6 p-5 items-baseline ${
            i !== rows.length - 1 ? "border-b border-primary-200/50" : ""
          }`}
        >
